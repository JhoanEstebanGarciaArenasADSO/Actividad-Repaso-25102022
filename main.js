        ...
        for (let val in data[dataInput.scale]) {
            if (data[dataInput.scale][val][0] <= dataInput.nota && data[dataInput.scale][val][1] >= dataInput.nota) {
                ...
                myTabla.insertAdjacentHTML("beforeend", `
                    ...
                    <tr>
                        <td colspan="5">${(val == 0) ? "Querido usuario su calificacion es de las <b>mejores</b>" : (val == 1) ? "Querido usuario se gano unas <b>felicitaciones</b>" : (val == 2) ? "Querido usuario usted es <b>basico</b>" : ""}</td>
                    </tr>
                `);
            }
        }
